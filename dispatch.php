...
<!---START OF STEP 10 IN THIS PROJECT 6 PROGRAM AND DEPLOY_V1.DOCX---->
					<!--Part 3 -->
<?php 	// Validate if request comes from logcall.php or post back
if (!isset($_POST["btnProcessCall"]) && !isset($_POST["btnDispatch"])) header("Location: logcall.php");
?>
...
if(isset($_POST["btnDispatch"]))
{
	...
	for($i=0; $i < $numOfPatrolcarDispatched; $i++)
	{
		...
		if (!$stmt->bind_param('ss', $incidentId, $ptrolcarDispatched[$i])){
			die("Binding parameters failed: ".$stmt->errno);
		}
	...
	$mysqli->close();
?>
<!-- After dispatching, redrict to logcall.php -->
<script type="text/javascript">window.loction="./logcall.php";</script>
<?php } ?>
</head>
<body>
...
<tr>
	<td colspan="2">Incident Detail</td>
		...
		<br></br>
		<!---End of step 6 in the Project 6 Program and deloy_v1.docx---->
		<!---Start of STEP 8.1 in the Project 6_Program and deloy_v1.docx---->
		<?php
	...
	if ($mysqli->connect_errno){
		...
		//retrieve from patrolcar table those patrol cars the are 2:Patrol or 3:Free
	...
	$mysqli->close();
	?>
	<!---END of STEP 8.1 in the project 6_Program and deloy_vl.docx---->
	<!---Start of STEP in the porject 6_Program and deloy_v1.docx---->
	...
	<!---END of STEP 8.2 in the project 6_Program and deloy_v1.docx---->
</form>
</body>
</html>